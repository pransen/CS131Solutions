...
def mix_images(image1, image2, channel1, channel2):
    ...
    img1 = rgb_exclusion(image1, channel1)
    img2 = rgb_exclusion(image2, channel2)
    img1[:, int(width/2):, :] = img2[:, int(width/2):, :]
    out = img1
    ### END YOUR CODE
...
def mix_quadrants(image):
    ...
    out[:int(height/2), :int(width/2), :] = tl[:int(height/2), :int(width/2), :]
    out[:int(height/2), int(width/2):, :] = tr[:int(height/2), int(width/2):, :]
    out[int(height/2):, :int(width/2), :] = bl[int(height/2):, :int(width/2), :]
    out[int(height/2):, int(width/2):, :] = br[int(height/2):, int(width/2):, :]
    ### END YOUR CODE
